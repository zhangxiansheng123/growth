...
7. 回表了解吗？
   * 回表就是先通过数据库索引扫描出数据所在的行，再通过行主键去除索引中未提供的数据，即基于非主键索引的查询需要多扫描一棵索引树。因此，可以通过索引先查询出id字段，在通过主键id字段，查询行中的字段数据，即通过再次查询提供sql查询速度。（只有主键索引的叶子结点有数据）。
8. mysql有缓存吗？
9. mysql的查询流程？
...
11. mysql索引分类？
    ...
    * 复合索引：即一个索引包含多个列。
12. 数据库表行列转换？
13. 我的id主键，自增长乱了怎么解决？索引失效了怎么办，比如like查询“%”的时候，怎么用最快的效率查询你想要的数据？
14. 往mysql里面插入数据，写入的时间和我系统的时间不一样，有两个小时的时间差，怎么办，怎么解决？
15. mysql和orcal有什么区别，你了解过吗？（我说引擎不同）
...
16. b+树知道吗？
17. 有十个数据，然后他让画一下b+树得存储结构？
18. 聊一下数据库索引？聚簇索引与非聚簇索引的区别？
...
19. Btree与B+tree的区别，b+tree叶子结点存储多少数据，b+tree的高度？
    * btree节点即存关键字又存记录，而b+tree只有叶子存储记录，非叶子节点只存key，这样存的key就多，降低了树的高度，提高了查询效率。
20. 查询一张表的数据经过几次io？
21. 一般怎么进行sql优化，怎么查看是否用了索引，explain主要需要关注那几个字段，type字段都有哪些值？
    * 使用explain
22. mysql怎么查看有没有用到索引？说说explain？explin需要注意哪些字段？type字段有哪些值？ref是什么级别？（我回答的用explain，然后他又问到怎么判断的，我说看type类型是否是index）
...
23. 有没有进行过mysql调优？sql如何优化？常用的sql优化有那些思路？

24. 组合索引是创建了几个索引？

25. 数据库索引有哪些 什么情况下失效 ，怎么造成索引失效！
...
26. 什么情况下mysql连接池会满？ mysql连接池满了怎么办？
...
31. SQL的having和where的区别（select distinct   from   where  group by having orderby）

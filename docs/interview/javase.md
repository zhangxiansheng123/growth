...
3. 数组和链表有什么区别？
   ...
   * 从内存存储角度来看：(静态)数组从栈中分配空间, 对于程序员方便快速,但自由度小。
   ...
   不同的操作对数组和链表时间复杂度的计算（数组和链表的时间复杂度是什么？）
...
39. 如何确保一个集合不会被修改
    ...
    * 通过Collectons.unmodifiableCollection(Collection c)方法。使用Arrays.asList()创建的集合。

40. hashmap能处理100W条数据吗，怎么将100万的数据快速高效的插入到map中？

41. hashmap你们会初始化长度吗 为什么呢？

42. 红黑树的数据结构是怎样的？[参考](https://www.cnblogs.com/ysocean/p/8004211.html)

43. 说一说排序算法冒泡排序、快速排序等？
